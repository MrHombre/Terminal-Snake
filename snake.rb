class GameItem
  ...
  def initialize(app, x, y, color = '#fff', fill = '000000')
    @app = app
    @x = x
    @y = y
    @app.stroke color
  ...
  # This removes the item from the game..I hope
  def remove
    @rect.hide
    true
  end
    ...
    def move
      @segments.reverse.each_with_index do |segment, i|
        if i + 1 == length
          ...
          next_segment = @segment.reverse[i + 1]
  ...
  class GameBoard
    class Food < GameItem
      def initialize(app, x, y, color = '#00ff00')
        super
      end
    end

    class Brick < GameItem
      def initialize(app, x, y, color = '#6666ff', fill = '#000099')
        super
      end
    end
    ...
    def add_food(x, y)
      # Dont add food over anything Lord please remember!
    ...
    # Add segment to the snake and remove food at the snake's head
    def eat_food
      @snake.add_segment
    ...
    def add_brick_border
      # Add bricks but dont record them for collision detection
